...
class BridgeTransport:
	...
	def make_callback(self, target_transport: Transport):
		def callback(callback_type, *args, **kwargs):  # clearer that this is the message/callback type
			if callback_type.value not in self.excluded:
				target_transport.send(callback_type, *args, **kwargs)
		return callback
